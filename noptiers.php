...
select LastUpdated, clientGroup, displayCcy, sum(tier1) as tier1NOP, sum(tier1Exposure) as tier1BaseExposure, sum(tier2) as tier2NOP, sum(tier2Exposure) as tier2BaseExposure, sum(tier3) as tier3NOP, sum(tier3Exposure) as tier3BaseExposure, sum(tier4) as tier4NOP, sum(tier4Exposure) as tier4BaseExposure, sum(tier5) as tier5NOP, sum(tier5Exposure) as tier5BaseExposure
from
(
...
";
        ...
        $sql2 = "select tier1,tier2,tier3,tier4,tier5 FROM do.nop_clients where clientGroup = '$clientFirm';";
    ...
    <h1 align="center">Net Open Positions - Tiers</h1>
        ...
        <div id="buttons">
            ...
            <a href='nopbreakdown.php?link=<?php echo $clientFirm;?>' class="btn">NOP Breakdown</a>
        </div> 
        ...
        <?php 
            if (mysql_num_rows($result_clientFirm) > 0)
            {
                ...
                while($row = mysql_fetch_assoc($result_clientFirm))
                {
                    ...
                    echo " <a href='nopTierBreakdownFirm.php?group=$clientFirm&firm=".$str[$c]."'>".$str[$c]."</a> | ";
                    $c++;
                }
            }
        ?> 
    ...
    <h2>NOP Tier Breakdown</h2>
    <table class="bordered">
        <thead>
            <tr>     
                <th style="text-align: left;" style="display: none;" width="80px"></th>
        ...
        <?php
            ...
            if (mysql_num_rows($result) > 0)
            {
                ...
                echo "<tr>";
                    echo "<td class='grey'><b>Total</b></b>";
                    ...
                    if ($percent1 >= 100.00)
                    {
                        echo "<td class='center-align' width='0px' style='background-color:#dc4814;'></td>";
                        echo "<td class='center-align' width='0px' style='background-color:#dc4814;'><b>".number_format($sum1)."</b></td>";
                    }elseif ($percent1 >= 75.00)
                    {
                        echo "<td class='center-align' width='0px' style='background-color:#e99e7c;'></td>";
